...
on both linux (Rocky 9 distro), macOS Sonoma 14.1, & macOS Ventura.
...
Usage
-----
NOTE running ***sheriff --help*** the first time is slow. 
...
#### Running Sheriff
***NOTE*** The Superb-seq paper included three sequencing libraries. Two of these libraries were on the same 
edited K562 cell lines, with edits generated by 7 guides targeting the exons of chromatin remodeler genes CHD3, CHD4, SMARCA4, and ARID1A. 
One library sequenced 10,000 cells at 100,000 reads/cell, and the other library sequenced 500 cells at 200,000 reads/cell.

The bam file used here is for the more deeply sequenced 500 cell library, with reads subsetted to those occurring
with 200kb of true edit sites that were identified in the 10,000 cell library. 

Therefore the edit site calling will be accurate, ***BUT*** the gene counts will NOT be
accurate, because most reads have been filtered from the example bam file. 
This filtering was performed so the smaller bam could be easily made available via github.

For more details on the Superb-seq libraries (including Sheriff outputs on GEO) and the inner processes of Sheriff, 
please refer to the manuscript listed at the end of this document.

#### Setting parameters
***NOTE*** 
For minimum cells to call canonical edit site, a minimum of 3 cells was used for the 10,000 library.
The CNV file input shown is only necessary if copy number variantion is present in the genome of the edited cells. 
Exclusion will result in 2 gene copies assumed throughout the genome. See the 'ploidy' parameter.
